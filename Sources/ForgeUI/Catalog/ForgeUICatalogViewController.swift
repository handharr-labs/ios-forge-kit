    ...
    private func makeSections() -> [CatalogSection] {
        [
            ...
            CatalogSection(title: "UIKit Components", items: [
                CatalogItem(title: "FUIAvatarView", makePreview: makeAvatarPreview),
                CatalogItem(title: "FUIBadgeView", makePreview: makeBadgePreview),
                CatalogItem(title: "FUILoadingView", makePreview: makeLoadingPreview),
                CatalogItem(title: "FUIMessageBubble", makePreview: makeMessageBubblePreview),
                CatalogItem(title: "FUIAudioPlayerView", makePreview: makeAudioPlayerPreview),
                CatalogItem(title: "FUIPrimaryButton", makePreview: makePrimaryButtonPreview),
                CatalogItem(title: "FUIEmptyStateView", makePreview: makeEmptyStatePreview),
            ]),
...
private extension ForgeUICatalogViewController {
    func makeAvatarPreview() -> UIView {
        ...
        stack.alignment = .center
        [FUIAvatarSize.small, .medium, .large].forEach { size in
            let avatar = FUIAvatarView()
            avatar.configure(with: FUIAvatarConfiguration(name: "Ada Lovelace", imageURL: nil, size: size))
            let label = UILabel()
            label.text = "\(size)"
            label.font = Typography.caption
            label.textColor = FUIColor.textSecondary
    ...
    func makeBadgePreview() -> UIView {
        ...
        [(0, "zero"), (5, "count"), (100, "capped")].forEach { count, label in
            let badge = FUIBadgeView()
            badge.configure(with: FUIBadgeConfiguration(count: count))
            let lbl = UILabel()
            lbl.text = label
            lbl.font = Typography.caption
            lbl.textColor = FUIColor.textSecondary
    ...
    func makeEmptyStatePreview() -> UIView {
        let view = FUIEmptyStateView()
        view.configure(with: FUIEmptyStateConfiguration(
