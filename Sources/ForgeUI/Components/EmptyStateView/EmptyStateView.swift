...
public final class FUIEmptyStateView: UIView {
    private let imageView: UIImageView = {
        let v = UIImageView()
        v.contentMode = .scaleAspectFit
        v.tintColor = FUIColor.textSecondary
    ...
    private let actionButton: FUIPrimaryButton = {
        let b = FUIPrimaryButton()
    ...
    public func configure(with config: FUIEmptyStateConfiguration) {
        imageView.image    = UIImage(systemName: config.systemImageName)
        titleLabel.text    = config.title
        subtitleLabel.text = config.subtitle

        if let buttonTitle = config.buttonTitle {
            actionButton.configure(with: FUIPrimaryButtonConfiguration(title: buttonTitle))
