...
final class TextMessageCell: UICollectionViewCell {
    private let bubble: FUIMessageBubble = {
        let v = FUIMessageBubble()
    ...
    func configure(with model: ChatUIModel) {
        ...
        let variant: FUIBubbleVariant = model.isOutgoing ? .outgoing : .incoming
        bubble.configure(with: FUIMessageBubbleConfiguration(
            text: text,
            variant: variant,
